...
- **Mobile sidebar**: On small screens, the hamburger menu includes accessibility controls in the sidebar footer—theme, language, reduce motion, and reset preferences—so users can adjust preferences without the desktop footer. Coverage: `tests/mobile-sidebar.spec.ts`.

## Testing

Accessibility is covered by automated tests in **`tests/accessibility.spec.ts`** (landmarks, focus, skip link, etc.), **`tests/footer.spec.ts`** (footer visibility, identity, theme/language/reduced-motion/reset controls, quick links, Back to top), and **`tests/mobile-sidebar.spec.ts`** (mobile sidebar accessibility controls: theme, language, reduce motion, reset preferences). The accessibility suite includes:
