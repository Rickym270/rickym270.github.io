...
- **Mobile sidebar**: On small screens, the hamburger menu includes a PREFERENCES section with Dark Mode and Reduce Motion toggle switches, language (EN/ES) pills, and footer icons (contact, reset preferences). Controls are keyboard-accessible and stay in sync with the desktop footer. Coverage: `tests/mobile-sidebar.spec.ts`.

## Testing

Accessibility is covered by automated tests in **`tests/accessibility.spec.ts`** (landmarks, focus, skip link, etc.), **`tests/footer.spec.ts`** (footer visibility, identity, theme/language/reduced-motion/reset controls, quick links, Back to top), and **`tests/mobile-sidebar.spec.ts`** (mobile sidebar: PREFERENCES, Dark Mode and Reduce Motion toggles, language, reset icon). The accessibility suite includes:
