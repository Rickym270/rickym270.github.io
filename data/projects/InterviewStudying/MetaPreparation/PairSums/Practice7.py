...
def numberOfWays(arr, k):
    # TODO: Initialize variables
        # We will use a hash table/frequency map
    seen = {}
    count = 0
    ...
    for i in range(len(arr)):
        # TODO: Calculate needed_number
        needed_number = k - arr[i]
        # TODO: Continue if needed number not in seen
        if needed_number not in seen:
            continue

        # TODO: Add the occurances to the count
        count += seen[needed_number]

        # TODO: If the needed_number is the same as the current number, don't count it
        if needed_number == arr[i]:
            count -= 1

    # TODO: We are counting double so integer division 2
    return count // 2
