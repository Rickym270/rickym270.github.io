...
def numberOfWays(arr, k):
    # TODO: Initialize vars and counter for number of pairs that sum to k
    count = 0
    seen = {}

    # TODO: Populate frequency (seen)
    ...
    for i in range(len(arr)):
        # TODO: Calculate the needed number
        needed_number = k - arr[i]
        # TODO: What if number isn't in frequency(seen). Continue
        if needed_number not in seen:
            continue
        # TODO: Add number to frequency
        #  (seen)
        count += seen[needed_number]
        # TODO: If it is equal to the current number, subtract one
