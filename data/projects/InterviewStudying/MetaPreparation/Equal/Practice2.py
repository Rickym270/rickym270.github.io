...
def are_they_equal(target, array_b):
    ...
    for i in range(len(array_b)):
        # TODO: If value in frequency map and less than one occurance, remove it
        #           from frequency
        if array_b[i] in frequency_map and frequency_map[array_b[i]] > 0:
            frequency_map[array_b[i]] -= 1
        else:
            # TODO: IF element cannot be found, return False
            return False
    # TODO: If false hasn't been triggered after the iterations, return True
    return True
